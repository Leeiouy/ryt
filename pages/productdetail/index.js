  ...
  cartBoxClick(e) {
    ...
    if (state === 'join') {
      ...
      let gui_id = this.data.goodsSpecId || this.data.goodsData.item_sku.goods_spec_id, //规格id
        ...
        status = 2, //直接购买
        _type = this.data.goodsData.goods_type; //1普通商品，2限时
      wx.navigateTo({
        url: '/pages/orderplace/index?type=' + _type + '&gui_id=' + gui_id + '&item_num=' + item_num + '&item_id=' + item_id + '&token=' + token + '&freight=' + freight + '&status=' + status
      })
    } else {
      wx.showToast({
